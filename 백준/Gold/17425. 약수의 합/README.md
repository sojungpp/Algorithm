...
1. dq와 g를 0으로 초기화한다.
2. A = BC 인 점을 이용하여, i마다 배수인 dp에 i의 값을 더해 각 숫자의 f(x)를 구해둔다.
3. g(x) = g(x-1) + f(x) 를 이용해 g(x)를 구한다.
4. 테스트 케이스의 개수만큼 for문을 통해 g(N)을 구하여 결과를 구한다.
