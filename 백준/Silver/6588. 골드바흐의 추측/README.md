...
1. 미리 정해진 범위만큼 배열을 만들어서 소수인지 체크해 True / False로 표시한다. (시간초과 해결)
2. `for a in range(3, len(prime), 2):` (시간초과 해결) <br>
n이 0(종료)임을 확인하고, 종료가 아니라면 3 ~ prime 수까지 2씩 증가시키면서 a가 홀수일 때만 (시간초과 해결) 체킹하도록 한다.
3. a와 n-a(b)가 모두 소수일 때 정답을 print하고 break 한다. 
